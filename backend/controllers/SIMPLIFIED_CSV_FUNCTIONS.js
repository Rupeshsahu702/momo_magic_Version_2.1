// SIMPLIFIED CSV FORMAT - Update both functions in employeeController.js

// 1. Replace downloadAttendanceTemplate function (around line 387-422)
const downloadAttendanceTemplate = async (req, res) => {
    try {
        ...
        // Prepare SIMPLIFIED CSV data - only essential columns
        ...
        // Configure CSV parser with simplified fields
...
// 2. Replace uploadAttendanceCSV function (around line 434-547)
const uploadAttendanceCSV = async (req, res) => {
    try {
        ...
        // Find column indices - SIMPLIFIED COLUMNS
        const dateIndex = headers.findIndex(h => h.toLowerCase().includes('date'));
        const employeeIdIndex = headers.findIndex(h => h.toLowerCase().includes('employee id'));
        const statusIndex = headers.findIndex(h => h.toLowerCase().includes('attendance status') || h.toLowerCase() === 'status');
        ...
        for (let i = 0; i < dataRows.length; i++) {
            ...
            const dateValue = dateIndex !== -1 ? values[dateIndex]?.trim() : todayString;
...
// Helper function to parse CSV row properly - ADD THIS AFTER uploadAttendanceCSV
